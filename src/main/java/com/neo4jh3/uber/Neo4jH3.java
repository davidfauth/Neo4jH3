...
public class Neo4jH3 {
    ...
    private final static String NEO4J_H3_VERSION = "2026.06.0";
    ...
    public Stream<H3LongAddress> gridPathCellsLonLat(@Name("polyEdges") List<String> polyEdges, @Name("h3Res") Long h3Res, @Name("latlonorder") String latlonorder) throws InterruptedException {
        ...
        try {
            if (h3Resolution > 0 && h3Resolution <= 15) {
                ...
                for (int i = 0; i < hexPoints.size(); i++) {
                    if (i <1 ){
                        int maxVal = hexPoints.size()-1;
                        try {
                            Double startLat = Precision.round(hexPoints.get(0).lat,6);
                            Double startLong = Precision.round(hexPoints.get(0).lng,6);
                            Double endLat = Precision.round(hexPoints.get(maxVal).lat,6);
                            Double endLong = Precision.round(hexPoints.get(maxVal).lng,6);
                            Long starthexAddr = h3.latLngToCell(startLat, startLong, h3Resolution);
                            Long endhexAddr = h3.latLngToCell(startLat, startLong, h3Resolution);
                            gpCells = h3.gridPathCells(starthexAddr, endhexAddr);
                            for (int j = 0; j < gpCells.size(); j++) {
                                if (!hexList.contains(gpCells.get(j))){
                                    hexList.add(gpCells.get(j));
                                }
                            }
                            gpCells.clear();  
                        } catch (Exception e1){
                        ...
                        try {
                            Double startLat = Precision.round(hexPoints.get(i-1).lat,6);
                            Double startLong = Precision.round(hexPoints.get(i-1).lng,6);
                            Double endLat = Precision.round(hexPoints.get(i).lat,6);
                            Double endLong = Precision.round(hexPoints.get(i).lng,6);
                            Long starthexAddr = h3.latLngToCell(startLat, startLong, h3Resolution);
                            Long endhexAddr = h3.latLngToCell(startLat, startLong, h3Resolution);
                            gpCells = h3.gridPathCells(starthexAddr, endhexAddr);
                            for (int j = 0; j < gpCells.size(); j++) {
                ...
                hexList = Collections.singletonList(-2L);
            
            }
        } catch (Exception e) {
            //System.out.println(e);
            hexList = Collections.singletonList(-1L);
            //e.printStackTrace();
        }
        return hexList.stream().map(H3LongAddress::of);
    }
    ...
    public Stream<H3StringAddress> gridPathCellsLonLatString(@Name("polyEdges") List<String> polyEdges, @Name("h3Res") Long h3Res, @Name("latlonorder") String latlonorder) throws InterruptedException {
        ...
        try {
            if (h3Resolution > 0 && h3Resolution <= 15) {
                ...
                for (int i = 0; i < hexPoints.size(); i++) {
                    if (i <1 ){
                        ...
                        try {
                            ...
                            Double endLat = Precision.round(hexPoints.get(maxVal).lat,6);
                            Double endLong = Precision.round(hexPoints.get(maxVal).lng,6);
                            String starthexAddr = h3.latLngToCellAddress(startLat, startLong, h3Resolution);
                ...
                hexList = Collections.singletonList("-2");
            
            }
        } catch (Exception e) {
            //System.out.println(e);
            hexList = Collections.singletonList("-1");
            //e.printStackTrace();
        }
        return hexList.stream().map(H3StringAddress::of);
    }
